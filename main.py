#!/usr/bin/env python3
"""
Main entry point for KYHUNTARR
...
root_logger = logging.getLogger("KYHUNTARRRoot") # Specific logger for this entry point
root_logger.info("--- KYHUNTARR Main Process Starting ---")
    ...
    if sys.argv[1] == '--install-service':
        ...
        try:
            ...
            if sys.argv[1] == '--start':
                ...
                from src.primary.windows_service import KYHUNTARRService
...
try:
    ...
    kyhuntarr_logger = setup_main_logger()
    kyhuntarr_logger.info("Successfully imported application components.")
...
def run_background_tasks():
    """Runs the KYHUNTARR background processing."""
    bg_logger = get_logger("KYHUNTARRBackground") # Use app's logger
    try:
        bg_logger.info("Starting KYHUNTARR background tasks...")
        start_kyhuntarr() # This function contains the main loop and shutdown logic
    except Exception as e:
        bg_logger.exception(f"Critical error in KYHUNTARR background tasks: {e}")
    finally:
        bg_logger.info("KYHUNTARR background tasks stopped.")
...
def main_shutdown_handler(signum, frame):
    ...
    kyhuntarr_logger.warning(f"Received signal {signal.Signals(signum).name}. Initiating shutdown...")
    if not stop_event.is_set():
        stop_event.set()
...
if __name__ == '__main__':
    ...
    try:
        ...
        background_thread = threading.Thread(target=run_background_tasks, name="KYHUNTARRBackground", daemon=True)
        ...
        kyhuntarr_logger.info("KeyboardInterrupt received in main thread. Shutting down...")
        if not stop_event.is_set():
            stop_event.set()
    except Exception as e:
        kyhuntarr_logger.exception(f"An unexpected error occurred in the main execution block: {e}")
        if not stop_event.is_set():
            stop_event.set() # Ensure shutdown is triggered on unexpected errors
    finally:
        # --- Cleanup ---
        kyhuntarr_logger.info("Web server has stopped. Initiating final shutdown sequence...")

        # Ensure the stop event is set (might already be set by signal handler or error)
        if not stop_event.is_set():
             kyhuntarr_logger.warning("Stop event was not set before final cleanup. Setting now.")
             stop_event.set()

        # Wait for the background thread to finish cleanly
        if background_thread and background_thread.is_alive():
            kyhuntarr_logger.info("Waiting for background tasks to complete...")
            background_thread.join(timeout=5) # Keep under Docker's 10s SIGKILL window

            if background_thread.is_alive():
                kyhuntarr_logger.warning("Background thread did not stop gracefully within the timeout.")
        elif background_thread:
             kyhuntarr_logger.info("Background thread already stopped.")
        else:
             kyhuntarr_logger.info("Background thread was not started.")

        # Call the shutdown_threads function from primary.main (if it does more than just join)
        # This might be redundant if start_kyhuntarr handles its own cleanup via stop_event
        # kyhuntarr_logger.info("Calling shutdown_threads()...")
        # shutdown_threads() # Uncomment if primary.main.shutdown_threads() does more cleanup

        kyhuntarr_logger.info("--- KYHUNTARR Main Process Exiting ---")
