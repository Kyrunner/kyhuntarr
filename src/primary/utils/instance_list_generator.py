...
# Set up logging
logger = logging.getLogger("kyhuntarr.instance_list_generator")
...
def generate_instance_list():
    ...
    try:
        ...
        logger.debug(
            f"Cannot create web-accessible directory: {e}. Will only save to config directory."
        )
    ...
    for app_type in app_types:
        ...
        try:
            ...
            if (
                config
                and "instances" in config
                and isinstance(config["instances"], list)
            ):
                # Add each instance to our instances object
                for index, instance in enumerate(config["instances"]):
                    instance_name = (
                        instance.get("name")
                        or f"{capitalize_first(app_type)} Instance {index + 1}"
                    )
                    instances[app_type].append(
                        {
                            ...
                            "display_name": app_display_names.get(
                                app_type, capitalize_first(app_type)
                            ),
                            "order": app_order.get(
                                app_type, 999
                            ),  # Use the map for strict ordering
                        }
                    )
    ...
    if web_accessible_path_valid:
        try:
            ...
            with open(web_list_file, "w") as f:
                json.dump(instances, f, indent=2)
            logger.debug(
                f"Instance list generated successfully at {list_file} and {web_list_file}"
            )
        except (PermissionError, OSError) as e:
