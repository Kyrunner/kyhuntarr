#!/usr/bin/env python3
"""
Settings migration utility for KYHUNTARR
...
SETTINGS_FILE = SETTINGS_DIR / "kyhuntarr.json"
...
def migrate_settings():
    ...
    if not SETTINGS_FILE.exists():
        logger.info(
            f"Settings file {SETTINGS_FILE} does not exist, nothing to migrate."
        )
        return
    ...
    try:
        ...
        for app in ["sonarr", "radarr", "lidarr"]:
            if app in settings and "kyhuntarr" in settings[app]:
                logger.info(f"Found nested kyhuntarr section in {app}, migrating...")

                # Move all settings from app.kyhuntarr to app level
                for key, value in settings[app]["kyhuntarr"].items():
                    if key not in settings[app]:
                        settings[app][key] = value
                        logger.info(f"Moved {app}.kyhuntarr.{key} to {app}.{key}")

                # Remove the kyhuntarr section
                del settings[app]["kyhuntarr"]
                logger.info(f"Removed {app}.kyhuntarr section")
                changes_made = True
        ...
        if changes_made:
            ...
            logger.info(
                "No changes needed, settings are already in the correct format."
            )
