#!/usr/bin/env python3
"""
Logging configuration for KYHUNTARR
...
MAIN_LOG_FILE = LOG_DIR / "kyhuntarr.log"
...
def setup_main_logger(debug_mode=None):
    """Set up the main KYHUNTARR logger."""
    global logger
    log_name = "kyhuntarr"
    ...
    log_format = "%(asctime)s - kyhuntarr - %(levelname)s - %(message)s"
...
def get_logger(app_type: str) -> logging.Logger:
    ...
    # Prevent propagation to the main 'kyhuntarr' logger or root logger
    ...
    log_format = f"%(asctime)s - kyhuntarr.{app_type} - %(levelname)s - %(message)s"
    formatter = logging.Formatter(log_format, datefmt="%Y-%m-%d %H:%M:%S")
...
def debug_log(
    message: str, data: object = None, app_type: Optional[str] = None
) -> None:
    """
