#!/usr/bin/env python3
"""
Scheduler Engine for KYHUNTARR
        ...
        if os.path.exists(SCHEDULE_FILE):
            try:
                ...
                if os.path.getsize(SCHEDULE_FILE) == 0:
                    return {"global": [], "sonarr": [], "radarr": [], "lidarr": []}

                # Attempt to load JSON
                with open(SCHEDULE_FILE, "r") as f:
                    content = f.read()
                    scheduler_logger.debug(
                        f"Schedule file content (first 100 chars): {content[:100]}..."
                    )
                    schedule_data = json.loads(content)

                    # Ensure the schedule data has the expected structure
                    for app_type in ["global", "sonarr", "radarr", "lidarr"]:
...
def execute_action(action_entry):
    ...
    try:
                ...
                try:
                    ...
                    for app in apps:
                        ...
                        if os.path.exists(config_file):
                            ...
                            if "instances" in config_data and isinstance(
                                config_data["instances"], list
                            ):
            ...
            try:
                ...
                if app_type == "global":
                    ...
                    try:
                        ...
                        error_message = (
                            f"Error setting global API cap to {api_limit}: {str(e)}"
                        )
                        scheduler_logger.error(error_message)
                        add_to_history(action_entry, "error", error_message)
                        return False
        ...
        scheduler_logger.error(
            f"Error executing action {action_type} for {app_type}: {e}"
        )
        scheduler_logger.error(traceback.format_exc())
        return False
...
def should_execute_schedule(schedule_entry):
    ...
    scheduler_logger.info(
        f"EXACT CURRENT TIME: {exact_time.strftime('%Y-%m-%d %H:%M:%S.%f')}"
    )
    ...
    scheduler_logger.info(
        f"CRITICAL DEBUG - Today: '{current_day}', Schedule days: {days}"
    )

    # If days array is empty, treat as "run every day"
    if not days:
        scheduler_logger.debug(
            f"Schedule {schedule_id} has no days specified, treating as 'run every day'"
        )
    else:
        ...
        if current_day not in lowercase_days:
            scheduler_logger.info(
                f"FAILURE: Schedule {schedule_id} not configured to run on {current_day}, skipping"
            )
            return False
        else:
            scheduler_logger.info(
                f"SUCCESS: Schedule {schedule_id} IS configured to run on {current_day}"
            )
    ...
    try:
        ...
        scheduler_logger.warning(
            f"Invalid schedule time format in entry: {schedule_entry}"
        )
        return False
    ...
    if current_time.hour == schedule_hour:
        ...
        if (
            ...
            and current_time.minute < schedule_minute + 4
        ):
            scheduler_logger.info(
...
def check_and_execute_schedules():
    ...
    try:
        ...
        schedule_summary = {
            app: len(schedules) for app, schedules in schedule_data.items()
        }
        scheduler_logger.debug(f"Loaded schedules: {schedule_summary}")

        # Add to history that we've checked schedules
        add_to_history(
            {"action": "check"}, "debug", f"Checking schedules at {current_time}"
        )
            ...
            for schedule_entry in schedules:
                ...
                if should_execute_schedule(schedule_entry):
                    ...
                    if entry_id and entry_id in last_executed_actions:
                        ...
                        if (
                            delta < 5
                        ):  # Don't re-execute if less than 5 minutes have passed
                            scheduler_logger.info(
...
def start_scheduler():
    ...
    scheduler_thread = threading.Thread(
        target=scheduler_loop, name="SchedulerEngine", daemon=True
    )
    ...
    scheduler_logger.info(
        f"Scheduler engine started. Thread is alive: {scheduler_thread.is_alive()}"
    )
    return True
