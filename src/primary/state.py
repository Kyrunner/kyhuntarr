#!/usr/bin/env python3
"""
State management module for KYHUNTARR
...
def check_state_reset(app_type: str = None) -> bool:
    ...
    reset_interval = settings_manager.get_advanced_setting(
    ...
    if hours_passed >= reset_interval:
        ...
        logger.warning(
            f"This will cause all previously processed media to be eligible for processing again"
        )
        ...
        if hours_passed >= (reset_interval * 2):
            logger.info(
                f"Confirmed state reset for {current_app_type} after {hours_passed:.1f} hours"
            )
...
def clear_processed_ids(app_type: str = None) -> None:
    ...
    try:
        ...
        logger.error(
            f"Error clearing processed missing IDs for {current_app_type}: {e}"
        )
    ...
    try:
        ...
        logger.error(
            f"Error clearing processed upgrade IDs for {current_app_type}: {e}"
        )
