...
from primary.utils.logger import (
    LOG_DIR,
    APP_LOG_FILES,
    MAIN_LOG_FILE,
...
)  # Import stateful update function
...
from src.primary.apps.blueprints import sonarr_bp, radarr_bp, lidarr_bp
...
print("==== KYHUNTARR TEMPLATE DEBUG ====")
...
KNOWN_LOG_FILES = {
    ...
    "system": MAIN_LOG_FILE,  # Map 'system' to the main kyhuntarr log
}
# Filter out None values if an app log file doesn't exist
KNOWN_LOG_FILES = {k: v for k, v in KNOWN_LOG_FILES.items() if v}

ALL_APP_LOG_FILES = list(
    KNOWN_LOG_FILES.values()
)  # List of all individual log file paths
...
def logs_stream():
    ...
    Supports 'all', 'system', 'sonarr', 'radarr', 'lidarr'.
    ...
    if app_type not in valid_app_types:
        web_logger.warning(
            f"Invalid app type '{app_type}' requested for logs. Defaulting to 'all'."
        )
        app_type = "all"
    ...
    web_logger.info(
        f"Starting log stream for app type: {app_type} (client: {client_id}, time: {current_time_str})"
    )
    ...
    with app.log_stream_lock:
        ...
        stale_clients = [
            c for c, t in app.active_log_streams.items() if current_time - t > 60
        ]
        for client in stale_clients:
            ...
            if client in app.active_log_streams:
                ...
                web_logger.debug(
                    f"Removed stale log stream connection for client: {client}"
                )
        ...
        web_logger.debug(
            f"Active log streams: {len(app.active_log_streams)} clients. Added/Updated: {client_id}"
        )
    ...
    def generate():
        ...
        web_logger.info(
            f"Log stream generator started for {app_type} (Client: {client_ip})"
        )
        try:
            ...
            if app_type == "all":
                ...
                if system_log:
                    ...
                    web_logger.debug(
                        f"Also following system log for {app_type} messages"
                    )
            ...
            while True:
                ...
                if current_time - last_activity > 10:
                    with app.log_stream_lock:
                        if client_id in app.active_log_streams:
                            ...
                            web_logger.debug(
                                f"Client {client_id} gone. Stopping generator."
                            )
                            break
                ...
                for name, path in log_files_to_follow:
                    try:
                        ...
                        if (
                            name in last_check
                            and (now - last_check[name]).total_seconds() < 0.2
                        ):
                            continue
                        ...
                        if not path.exists():
                            if positions.get(name) != -1:
                                web_logger.warning(
                                    f"Log file {path} not found. Skipping."
                                )
                                positions[name] = -1
                            continue
                        ...
                        try:
                            ...
                            web_logger.warning(
                                f"Log file {path} disappeared. Skipping."
                            )
                            positions[name] = -1
                            continue

                        # Init position or handle truncation
                        if name not in positions or current_size < positions.get(
                            name, 0
                        ):
                        ...
                        try:
                            ...
                            web_logger.warning(
                                f"Log file {path} disappeared during read."
                            )
                            positions[name] = -1
                        except Exception as e:
            ...
            web_logger.info(
                f"Client {client_id} disconnected from log stream for {app_type}. Cleaning up."
            )
        except Exception as e:
            web_logger.error(
                f"Unhandled error in log stream generator for {app_type} (Client: {client_ip}): {e}",
                exc_info=True,
            )
            try:
                ...
                web_logger.error(
                    f"Error yielding final error message to client {client_id}: {yield_err}"
                )
        finally:
            ...
            with app.log_stream_lock:
                ...
                if removed_client:
                    web_logger.info(
                        f"Successfully removed client {client_id} from active log streams."
                    )
                else:
                    web_logger.debug(
            ...
            web_logger.info(
                f"Log stream generator finished for {app_type} (Client: {client_id})"
            )

    # Return the SSE response with appropriate headers for better streaming
    response = Response(
        stream_with_context(generate()), mimetype="text/event-stream"
    )  # Use stream_with_context
    response.headers["Cache-Control"] = "no-cache"
    response.headers["X-Accel-Buffering"] = (
        "no"  # Disable nginx buffering if using nginx
    )
    return response
...
def save_general_settings():
    ...
    if success:
        ...
        try:
            ...
            if new_hours > 0:
                general_logger.info(
                    f"Updating stateful expiration to {new_hours} hours."
                )
        ...
        return jsonify(
            {"success": False, "error": "Failed to save general settings"}
        ), 500
...
def handle_app_settings(app_name):
    ...
    if app_name not in settings_manager.KNOWN_APP_TYPES:
        return jsonify(
            {"success": False, "error": f"Unknown application type: {app_name}"}
        ), 400
    ...
    if request.method == "GET":
        ...
        if "instances" in data and isinstance(data["instances"], list):
            for instance in data["instances"]:
                if "api_url" in instance and instance["api_url"]:
                    ...
                    instance["api_url"] = (
                        instance["api_url"].strip().rstrip("/").rstrip("\\")
                    )
        ...
        if success:
            ...
            return jsonify(
                {"success": False, "error": f"Failed to save {app_name} settings"}
            ), 500
...
def api_reset_settings():
    ...
    if (
        not app_name or app_name not in settings_manager.KNOWN_APP_TYPES
    ):  # Corrected attribute name
        return jsonify(
            {"success": False, "error": f"Invalid or missing app name: {app_name}"}
        ), 400
    ...
    if success:
        ...
        return jsonify(
            {"success": False, "error": f"Failed to save reset settings for {app_name}"}
        ), 500
...
def api_app_settings():
    ...
    if (
        not app_type or app_type not in settings_manager.KNOWN_APP_TYPES
    ):  # Corrected attribute name
        return jsonify(
            {"success": False, "error": f"Invalid or missing app type: {app_type}"}
        ), 400
...
def api_configured_apps():
    ...
    configured_apps_list = (
        settings_manager.get_configured_apps()
    )  # Corrected function name
    # Convert list to dict format expected by frontend
    configured_status = {
        app: (app in configured_apps_list) for app in settings_manager.KNOWN_APP_TYPES
    }
...
def api_app_status(app_name):
    ...
    if app_name not in settings_manager.KNOWN_APP_TYPES:
        ...
        return jsonify(
            {"configured": False, "connected": False, "error": "Invalid app name"}
        ), 400

    try:
        if app_name in ["sonarr", "radarr", "lidarr"]:
            ...
            try:
                ...
                if hasattr(instances_module, "get_configured_instances"):
                    get_instances_func = getattr(
                        instances_module, "get_configured_instances"
                    )
                    instances = get_instances_func()
                    total_configured = len(instances)
                    api_timeout = settings_manager.get_setting(
                        app_name, "api_timeout", 10
                    )  # Get global timeout
                    ...
                    if total_configured > 0:
                        ...
                        if hasattr(api_module, "check_connection"):
                            check_connection_func = getattr(
                                api_module, "check_connection"
                            )
                            for instance in instances:
                                ...
                                try:
                                    ...
                                    if check_connection_func(
                                        inst_url, inst_key, min(api_timeout, 5)
                                    ):
                                        web_logger.debug(
                            ...
                            web_logger.warning(
                                f"check_connection function not found in {app_name} API module"
                            )
                    else:
                        web_logger.debug(
                            f"No configured {app_name.capitalize()} instances found for status check."
                        )
                    ...
                    web_logger.warning(
                        f"get_configured_instances function not found in {app_name} module"
                    )
                    ...
                    if is_configured and hasattr(api_module, "check_connection"):
                        ...
                        is_connected = check_connection_func(
                            api_url, api_key, min(api_timeout, 5)
                        )
                    response_data = {
                ...
                web_logger.error(
                    f"Failed to import {app_name} modules for status check: {e}"
                )
                response_data = {
                    "total_configured": 0,
                    "connected_count": 0,
                ...
                web_logger.error(
                    f"Error during {app_name} multi-instance status check: {e}",
                    exc_info=True,
                )
                response_data = {
                    "total_configured": total_configured,
            ...
            if is_configured:
                try:
                    ...
                    if hasattr(api_module, "check_connection"):
                        ...
                        web_logger.warning(
                            f"check_connection function not found in {module_path}"
                        )
                except ImportError:
                    ...
                    web_logger.error(
                        f"Error checking connection for {app_name}: {str(e)}"
                    )
        ...
        web_logger.error(
            f"Unexpected error in status check for {app_name}: {str(e)}", exc_info=True
        )
        # Return a valid response even on error to prevent UI issues
        return jsonify(
            {"configured": False, "connected": False, "error": "Internal error"}
        ), 200
...
def api_start_hunt():
    ...
    return jsonify(
        {
            "success": True,
            "message": "Hunt control endpoint (start) - functionality may change.",
        }
    )
...
def api_stop_hunt():
    ...
    return jsonify(
        {
            "success": True,
            "message": "Hunt control endpoint (stop) - functionality may change.",
        }
    )
...
def api_reset_stats():
    ...
    try:
        ...
        if reset_success:
            return jsonify(
                {"success": True, "message": "Statistics reset successfully"}
            )
        else:
            return jsonify(
...
def api_get_hourly_caps():
    ...
    try:
        ...
        for app in apps:
            ...
            app_limits[app] = app_settings.get(
                "hourly_cap", 20
            )  # Default to 20 if not set

        web_logger.debug(
            f"Serving hourly caps data with app-specific limits: {app_limits}"
        )
        ...
        return jsonify(
            {"success": False, "message": "Error retrieving hourly API caps."}
        ), 500
...
def version_txt():
    ...
    try:
        ...
        if os.path.exists(version_path):
            ...
            web_logger = get_logger("web_server")
            web_logger.warning(
                f"version.txt not found at {version_path}, returning default version"
            )
            return (
                "5.3.1",
                200,
...
def reset_app_cycle(app_name):
    ...
    Args:
        app_name: The name of the app (sonarr, radarr, lidarr)
    ...
    if app_name not in ["sonarr", "radarr", "lidarr"]:
        return jsonify(
            {"success": False, "error": f"Invalid app name: {app_name}"}
        ), 400
    ...
    if app_name not in configured_apps:
        return jsonify(
            {"success": False, "error": f"{app_name} is not configured"}
        ), 400
    ...
    try:
        ...
        web_logger.error(
            f"Error creating reset file for {app_name}: {e}", exc_info=True
        )
    ...
    if success:
        return jsonify(
            {"success": True, "message": f"Cycle reset triggered for {app_name}"}
        )
    else:
        return jsonify(
            {
...
def start_web_server():
    ...
    web_logger.info(
        f"Attempting to start web server on {host}:{port} (Debug: {debug_mode})"
    )  # Modified log
    # In production, use Werkzeug's simple server or a proper WSGI server
    web_logger.info("--- Calling app.run() ---")  # Added log
    app.run(
        host=host, port=port, debug=debug_mode, use_reloader=False
    )  # Keep this line if needed for direct execution testing, but it's now handled by root main.py
