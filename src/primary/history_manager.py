...
def add_history_entry(app_type, entry_data):
    ...
    logger.debug(
        f"Adding history entry for {app_type} with instance_name: '{instance_name}'"
    )
    ...
    logger.info(
        f"Added history entry for {app_type}-{instance_name}: {entry_data['name']}"
    )
    return entry
...
def get_history(app_type, search_query=None, page=1, page_size=20):
                ...
                for history_file in app_dir.glob("*.json"):
                    try:
                        with open(history_file, "r") as f:
                            ...
                            logger.debug(
                                f"Read {len(instance_history)} entries from {history_file}"
                            )
                    except (json.JSONDecodeError, FileNotFoundError) as e:
                        logger.warning(
                            f"Error reading instance history file {history_file}: {str(e)}"
                        )
    else:
        ...
        if app_dir.exists():
            ...
            for history_file in instance_files:
                try:
                    ...
                    logger.warning(
                        f"Error reading instance history file {history_file}: {e}"
                    )
    ...
    total_pages = (
        (total_entries + page_size - 1) // page_size if total_entries > 0 else 1
    )
        ...
        if app_type == "all":
            ...
            for app in history_locks.keys():
                ...
                if app_dir.exists():
                    instance_files = list(app_dir.glob("*.json"))
                    logger.debug(
                        f"Found {len(instance_files)} instance files to clear for {app}"
                    )
                    ...
                    for history_file in instance_files:
                        with open(history_file, "w") as f:
                            json.dump([], f)
                            logger.debug(
                                f"Cleared instance history file: {history_file}"
                            )
        else:
            ...
            if app_dir.exists():
                instance_files = list(app_dir.glob("*.json"))
                logger.debug(
                    f"Found {len(instance_files)} instance files to clear for {app_type}"
                )
...
def handle_instance_rename(app_type, old_instance_name, new_instance_name):
    ...
    logger.info(
        f"Handling instance rename for {app_type}: {old_instance_name} -> {new_instance_name}"
    )
        ...
        try:
            ...
            if old_file.exists():
                try:
                    ...
                    logger.info(
                        f"Loaded {len(old_data)} history entries from {old_file}"
                    )
                except (json.JSONDecodeError, FileNotFoundError) as e:
            ...
            if new_file.exists():
                try:
                    ...
                    logger.info(
                        f"Loaded {len(new_data)} existing history entries from {new_file}"
                    )
                except (json.JSONDecodeError, FileNotFoundError) as e:
            ...
            existing_keys = {
                (entry.get("id", ""), entry.get("date_time", 0)) for entry in new_data
            }
            for entry in old_data:
            ...
            # Sort by timestamp
            new_data = sorted(
                new_data, key=lambda x: x.get("date_time", 0), reverse=True
            )
    ...
    try:
        ...
        if not history_file.exists():
            with open(history_file, "w") as f:
                json.dump([], f)
            logger.info(
                f"Created history file for {app_type}/{instance_name}: {history_file}"
            )
            ...
            if instances_dir.exists():
                for instance_file in instances_dir.glob("*.json"):
                    try:
                        ...
                        logger.error(
                            f"Error processing instance file {instance_file}: {e}"
                        )
