#!/usr/bin/env python3
"""
Settings manager for KYHUNTARR
...
DEFAULT_CONFIGS_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "default_configs")
)

# Update or add this as a class attribute or constant
KNOWN_APP_TYPES = ["sonarr", "radarr", "lidarr", "general"]
...
def get_settings_file_path(app_name: str) -> pathlib.Path:
    ...
    if app_name not in KNOWN_APP_TYPES:
        ...
        settings_logger.warning(
            f"Requested settings file for unknown app type: {app_name}"
        )
    ...
    if default_file.exists():
        try:
            ...
            settings_logger.error(
                f"Error loading default settings for {app_name} from {default_file}: {e}"
            )
            return {}
    else:
        settings_logger.warning(
            f"Default settings file not found for {app_name}: {default_file}"
        )
        return {}
        ...
        if default_file.exists():
            try:
                ...
                settings_logger.info(
                    f"Created default settings file for {app_name} at {settings_file}"
                )
            except Exception as e:
                settings_logger.error(
                    f"Error copying default settings for {app_name}: {e}"
                )
        else:
            # Create an empty file if no default exists
            settings_logger.warning(
                f"No default config found for {app_name}. Creating empty settings file."
            )
            try:
                ...
                settings_logger.error(
                    f"Error creating empty settings file for {app_name}: {e}"
                )
...
def load_settings(app_type, use_cache=True):
    ...
    if app_type not in KNOWN_APP_TYPES and app_type != "general":
        settings_logger.warning(
            f"load_settings called with unexpected app_type: {app_type}"
        )
    ...
    if use_cache and app_type in settings_cache:
        ...
        if cache_age < CACHE_TTL:
            settings_logger.debug(
                f"Using cached settings for {app_type} (age: {cache_age:.1f}s)"
            )
            return cache_entry["data"]
        else:
            settings_logger.debug(
                f"Cache expired for {app_type} (age: {cache_age:.1f}s)"
            )
    ...
    try:
        with open(settings_file, "r") as f:
            ...
            if updated:
                ...
                save_settings(
                    app_type, current_settings
                )  # Use save_settings to handle writing
        ...
        settings_logger.error(
            f"Error decoding JSON from {settings_file}. Restoring from default."
        )
        ...
        settings_logger.error(
            f"Error loading settings for {app_type} from {settings_file}: {e}"
        )
...
def save_settings(app_name: str, settings_data: Dict[str, Any]) -> bool:
    ...
    if app_name not in KNOWN_APP_TYPES:
        settings_logger.error(
            f"Attempted to save settings for unknown app type: {app_name}"
        )
        return False
    ...
    try:
        ...
        settings_logger.info(
            f"Settings saved successfully for {app_name} to {settings_file}"
        )
        ...
        settings_logger.error(
            f"Error saving settings for {app_name} to {settings_file}: {e}"
        )
        return False
...
def get_configured_apps() -> List[str]:
    ...
    for app_name in KNOWN_APP_TYPES:
        ...
        if (
            ...
            and isinstance(settings["instances"], list)
            and settings["instances"]
        ):
            for instance in settings["instances"]:
                if (
                    instance.get("enabled", True)
                    and instance.get("api_url")
                    and instance.get("api_key")
                ):
...
if __name__ == "__main__":
    ...
    settings_logger.info(
        f"Loaded Sonarr settings: {json.dumps(sonarr_settings, indent=2)}"
    )
    ...
    if sonarr_settings:
        ...
        settings_logger.info(
            f"Reloaded Sonarr settings after save: {json.dumps(reloaded_sonarr_settings, indent=2)}"
        )

    # Test getting all settings
    all_app_settings = get_all_settings()
    settings_logger.info(
        f"All loaded settings: {json.dumps(all_app_settings, indent=2)}"
    )
