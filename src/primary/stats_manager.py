#!/usr/bin/env python3
"""
Statistics Manager for KYHUNTARR
...
STATS_DIRS = [
    ...
    os.path.join(os.path.expanduser("~"), ".kyhuntarr/tally"),  # User's home directory
    os.path.join(
...
def load_hourly_caps() -> Dict[str, Dict[str, int]]:
    ...
    try:
        if os.path.exists(HOURLY_CAP_FILE):
            ...
            logger.info(
                f"Hourly caps file not found at {HOURLY_CAP_FILE}, using default caps"
            )
            return default_caps
    except Exception as e:
...
def save_hourly_caps(caps: Dict[str, Dict[str, int]]) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Error saving hourly caps to {HOURLY_CAP_FILE}: {e}", exc_info=True
        )
        return False
...
def reset_hourly_caps() -> bool:
    ...
    try:
        if os.path.exists(HOURLY_CAP_FILE):
            ...
            logger.info(
                f"Deleted hourly caps file at {HOURLY_CAP_FILE} to reset all API caps"
            )
...
def increment_hourly_cap(app_type: str, count: int = 1) -> bool:
    ...
    with hourly_lock:
        ...
        if new_value >= int(hourly_limit * 0.8) and prev_value < int(
            hourly_limit * 0.8
        ):
            logger.warning(
                f"{app_type} is approaching hourly API cap: {new_value}/{hourly_limit}"
            )

        # Alert if exceeding limit
        if new_value >= hourly_limit and prev_value < hourly_limit:
            logger.error(
                f"{app_type} has exceeded hourly API cap: {new_value}/{hourly_limit}"
            )
...
def get_hourly_cap_status(app_type: str) -> Dict[str, Any]:
    ...
    with hourly_lock:
        ...
        return {
            ...
            "percent_used": int((current_usage / hourly_limit) * 100)
            if hourly_limit > 0
            else 0,
...
def increment_stat(app_type: str, stat_type: str, count: int = 1) -> bool:
    ...
    with stats_lock:
        ...
        logger.info(
            f"*** STATS INCREMENT *** {app_type} {stat_type} by {count}: {prev_value} -> {new_value}"
        )
        save_success = save_stats(stats)

        if not save_success:
            logger.error(
                f"Failed to save stats after incrementing {app_type} {stat_type}"
            )
            return False
