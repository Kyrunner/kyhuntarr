#!/usr/bin/env python3
"""
Common routes blueprint for KYHUNTARR web interface
"""
...
from ..auth import (
    ...
    generate_2fa_secret,
    verify_2fa_code,
    is_2fa_enabled,  # Add missing auth imports
)
...
def favicon():
    return send_from_directory(
        common_bp.static_folder, "favicon.ico", mimetype="image/vnd.microsoft.icon"
    )
...
def login_route():
    if request.method == "POST":
        try:  # Wrap the POST logic in a try block for better error handling
            ...
            twoFactorCode = data.get(
                "twoFactorCode"
            )  # Changed from 'otp_code' to match frontend form
            ...
            logger.debug(
                f"Auth result for '{username}': success={auth_success}, needs_2fa={needs_2fa}"
            )
            ...
            if auth_success:
                ...
                session[SESSION_COOKIE_NAME] = (
                    session_token  # Store token in Flask session immediately
                )
                response = jsonify(
                ...
                logger.warning(
                    f"Login failed for '{username}': 2FA required or invalid."
                )
                logger.debug(
                    f'Returning 2FA required response: {{"success": False, "requires_2fa": True, "requiresTwoFactor": True, "error": "Invalid or missing 2FA code"}}'
                )
                ...
                logger.warning(
                    f"Login failed for '{username}': Invalid credentials or other error."
                )
                return jsonify(
            ...
            logger.error(
                f"Unexpected error during login POST for user '{username if 'username' in locals() else 'unknown'}': {e}",
                exc_info=True,
            )
            return jsonify(
                {
...
def logout_route():
    try:
        ...
        if session_token:
            logger.info(
                f"Logging out session token: {session_token[:8]}..."
            )  # Log part of token
        ...
        response.delete_cookie(
            SESSION_COOKIE_NAME, path="/", samesite="Lax"
        )  # Specify path and samesite
...
def setup():
    if user_exists():  # This function should now be defined via import
        ...
        logger.info(
            "Setup page accessed but user already exists. Redirecting to login."
        )
    ...
    if request.method == "POST":
        ...
        try:  # Add try block to catch potential errors during user creation
            ...
            proxy_auth_bypass = data.get(
                "proxy_auth_bypass", False
            )  # Get proxy auth bypass setting
            ...
            if create_user(
                username, password
            ):  # This function should now be defined via import
                ...
                if proxy_auth_bypass:
                    try:
                        ...
                        logger.error(
                            f"Error saving proxy auth bypass setting: {e}",
                            exc_info=True,
                        )

                # Automatically log in the user after setup
                logger.info(
                    f"User '{username}' created successfully during setup. Creating session."
                )
                session_token = create_session(username)
                ...
                logger.error(
                    f"create_user function returned False for user '{username}' during setup."
                )
                return jsonify(
                    {
                        "success": False,
                        "error": "Failed to create user (internal reason)",
                    }
                ), 500
        except Exception as e:
            # Catch any unexpected exception during the process
            logger.error(
                f"Unexpected error during setup POST for user '{username if username else 'unknown'}': {e}",
                exc_info=True,
            )
            return jsonify(
                {"success": False, "error": f"An unexpected server error occurred: {e}"}
            ), 500
    else:
...
def get_user_info_route():
    ...
    if not username:
        logger.warning(
            "Attempt to get user info failed: Not authenticated (no valid session)."
        )
        return jsonify({"error": "Not authenticated"}), 401

    # Pass username to is_2fa_enabled
    two_fa_status = is_2fa_enabled(
        username
    )  # This function should now be defined via import
...
def change_username_route():
    ...
    if auth_change_username(current_username, new_username, password):
        ...
        logger.info(
            f"Username changed successfully for '{current_username}' to '{new_username}'."
        )
        ...
        return jsonify(
            {"success": True, "username": updated_username}
        )  # Return new username
    else:
        logger.warning(
            f"Username change failed for '{current_username}'. Check logs in auth.py for details."
        )
        return jsonify(
            {
                "success": False,
                "error": "Failed to change username. Check password or logs.",
            }
        ), 400
...
def change_password_route():
    ...
    if not current_password or not new_password:
        logger.warning(
            f"Password change attempt for user '{username}' failed: Missing current or new password."
        )
        return jsonify(
    ...
    if auth_change_password(current_password, new_password):
        ...
        logger.warning(
            f"Password change failed for user '{username}'. Check logs in auth.py for details."
        )
        return jsonify(
            {
                "success": False,
                "error": "Failed to change password. Check current password or logs.",
            }
        ), 400
...
def setup_2fa():
    ...
    if not username:
        logger.warning(
            "2FA setup attempt failed: No username in session."
        )  # Add logging
        return jsonify({"error": "Not authenticated"}), 401
    ...
    try:
        ...
        secret, qr_code_data_uri = generate_2fa_secret(
            username
        )  # This function should now be defined via import

        # Return secret and QR code data URI
        return jsonify(
            {"success": True, "secret": secret, "qr_code_url": qr_code_data_uri}
        )  # Match frontend expectation 'qr_code_url'

    except Exception as e:
        logger.error(
            f"Error during 2FA setup generation for user '{username}': {e}",
            exc_info=True,
        )
        return jsonify(
            {"success": False, "error": "Failed to generate 2FA setup information."}
        ), 500
...
def verify_2fa():
    ...
    if not username:
        logger.warning(
            "2FA verify attempt failed: No username in session."
        )  # Add logging
        return jsonify({"error": "Not authenticated"}), 401
    ...
    if not otp_code or len(otp_code) != 6 or not otp_code.isdigit():  # Add validation
        logger.warning(
            f"2FA verification for '{username}' failed: Invalid code format provided."
        )
        return jsonify(
            {"success": False, "error": "Invalid or missing 6-digit OTP code"}
        ), 400

    logger.info(f"Attempting to verify 2FA code for user '{username}'.")
    # Pass username to verify_2fa_code
    if verify_2fa_code(
        username, otp_code, enable_on_verify=True
    ):  # This function should now be defined via import
        logger.info(
            f"Successfully verified and enabled 2FA for user: {username}"
        )  # Add logging
        return jsonify({"success": True})
    else:
        # Reason logged in verify_2fa_code
        logger.warning(
            f"2FA verification failed for user: {username}. Check logs in auth.py."
        )
        return jsonify(
...
def disable_2fa_route():
    ...
    if not password or not otp_code:
        logger.warning(
            f"2FA disable attempt for '{username}' failed: Missing password or OTP code."
        )
        return jsonify(
            {
                "success": False,
                "error": "Both password and current OTP code are required to disable 2FA",
            }
        ), 400

    if not (len(otp_code) == 6 and otp_code.isdigit()):
        logger.warning(
            f"2FA disable attempt for '{username}' failed: Invalid OTP code format."
        )
        return jsonify(
            {"success": False, "error": "Invalid 6-digit OTP code format"}
        ), 400

    # Call a function that verifies both password and OTP
    if disable_2fa_with_password_and_otp(username, password, otp_code):
        logger.info(
            f"2FA disabled successfully for user '{username}' using password and OTP."
        )
        return jsonify({"success": True})
    else:
        # Reason logged in disable_2fa_with_password_and_otp
        logger.warning(
            f"Failed to disable 2FA for user '{username}' using password and OTP. Check logs."
        )
        ...
        return jsonify(
            {
                "success": False,
                "error": "Failed to disable 2FA. Invalid password or OTP code.",
            }
        ), 400
...
def set_theme():
    ...
    try:
        ...
        if dark_mode is None or not isinstance(dark_mode, bool):
            ...
            return jsonify(
                {"success": False, "error": "Invalid 'dark_mode' value"}
            ), 400
...
def get_local_access_bypass_status_route():
    ...
    try:
        ...
        local_access_bypass = settings_manager.get_setting(
            "general", "local_access_bypass", False
        )
        proxy_auth_bypass = settings_manager.get_setting(
            "general", "proxy_auth_bypass", False
        )
        ...
        logger.debug(
            f"Retrieved bypass status: local={local_access_bypass}, proxy={proxy_auth_bypass}, combined={bypass_enabled}"
        )
...
def reset_stats_api():
    ...
    try:
        ...
        if app_type is not None and app_type not in ["sonarr", "radarr", "lidarr"]:
        ...
        if reset_stats(app_type):
            message = (
                f"Reset statistics for {app_type}"
                if app_type
                else "Reset all statistics"
            )
            logger.info(message)
            return jsonify({"success": True, "message": message})
        else:
            error_msg = (
                f"Failed to reset statistics for {app_type}"
                if app_type
                else "Failed to reset all statistics"
            )
