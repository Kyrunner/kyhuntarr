...
def arr_request(
    ...
    try:
        ...
        if response.status_code == 401:
            logger.error(
                f"API request error: 401 Client Error: Unauthorized for url: {url}"
            )
            return None
...
def wait_for_command(command_id: int):
    ...
    while True:
        try:
            ...
            logger.error(
                f"Error fetching command status on attempt {attempts + 1}: {error}"
            )
            return False
        ...
        if (
            response["status"].lower() in ["complete", "completed"]
    ...
    if response["status"].lower() not in ["complete", "completed"]:
        logger.warning(
            f"Command {command_id} did not complete within the allowed attempts."
        )
        return False
...
def get_series() -> List[Dict]:
    ...
    if series_list:
        debug_log(
            "Raw series API response sample:",
...
def get_series_with_missing_episodes() -> List[Dict]:
    ...
    if APP_TYPE != "sonarr":
        logger.error(
            "get_series_with_missing_episodes() called but APP_TYPE is not sonarr"
        )
        return []
    ...
    if not missing_data or "records" not in missing_data:
        logger.error(
            "Failed to get missing episodes data or no 'records' field in response"
        )
        return []
        ...
        if "series" in episode and isinstance(episode["series"], dict):
            ...
            if series_id not in series_with_missing:
                ...
                if series_info:
                    ...
                    logger.warning(
                        f"Could not get series info for ID {series_id}, skipping episode"
                    )
                    continue
    ...
    logger.debug(
        f"Processed missing episodes data into {len(result)} series with missing episodes"
    )
    return result
...
def get_media_stats():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": "Error retrieving media statistics."}
        ), 500
...
def reset_media_stats():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": "Error resetting media statistics."}
        ), 500
