#!/usr/bin/env python3
"""
Stateful Manager for KYHUNTARR
...
APP_TYPES = ["sonarr", "radarr", "lidarr"]
...
def initialize_lock_file() -> None:
    ...
    if not LOCK_FILE.exists():
        try:
            ...
            expiration_hours = get_advanced_setting(
                "stateful_management_hours", DEFAULT_HOURS
            )
            ...
            with open(LOCK_FILE, "w") as f:
                json.dump(
                    {"created_at": current_time, "expires_at": expires_at}, f, indent=2
                )
...
def get_lock_info() -> Dict[str, Any]:
    ...
    try:
        ...
        if "expires_at" not in lock_info or lock_info["expires_at"] is None:
            ...
            lock_info["expires_at"] = lock_info["created_at"] + (
...
def update_lock_expiration(hours: int = None) -> bool:
    ...
    try:
        with open(LOCK_FILE, "w") as f:
            json.dump(lock_info, f, indent=2)
        stateful_logger.info(
            f"Updated lock expiration to {datetime.datetime.fromtimestamp(expires_at)}"
        )
        return True
    except Exception as e:
...
def get_processed_ids(app_type: str, instance_name: str) -> Set[str]:
    ...
    stateful_logger.debug(
        f"[get_processed_ids] Checking file: {file_path} for {app_type}/{instance_name}"
    )  # DEBUG LOG

    if not file_path.exists():
        stateful_logger.debug(
            f"[get_processed_ids] File not found: {file_path}"
        )  # DEBUG LOG
        return set()
    ...
    try:
        ...
        stateful_logger.error(
            f"Error reading processed IDs for {instance_name} from {file_path}: {e}"
        )  # Updated log
        return set()
...
def add_processed_id(app_type: str, instance_name: str, media_id: str) -> bool:
    ...
    try:
        with open(file_path, "w") as f:
            json.dump(
                {"processed_ids": processed_ids_list, "last_updated": int(time.time())},
                f,
                indent=2,
            )
...
def initialize_stateful_system():
    ...
    try:
        ...
        stateful_logger.info(
            f"Stateful lock file initialized with {expiration_hours} hour expiration"
        )
    except Exception as e:
