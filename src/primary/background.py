#!/usr/bin/env python3
"""
KYHUNTARR - Main entry point for the application
...
def app_specific_loop(app_type: str) -> None:
    ...
    Args:
        app_type: The type of Arr application (sonarr, radarr, lidarr)
    """
    ...
    try:
        ...
        app_logger.debug(
            f"Attributes found in {app_module.__name__}: {dir(app_module)}"
        )
        ...
        try:
            ...
            app_logger.debug(
                f"Found 'get_configured_instances' in {app_module.__name__}"
            )
        ...
        get_queue_size = getattr(
            ...
            lambda api_url, api_key, api_timeout: 0,
        ...
        app_logger.error(
            f"Failed to import modules or functions for {app_type}: {e}", exc_info=True
        )
        return  # Exit thread if essential modules fail to load

    # Create app-specific logger using provided function
    app_logger = logging.getLogger(f"kyhuntarr.{app_type}")
    ...
    while not stop_event.is_set():
        ...
        if get_instances_func:
            ...
            try:
                ...
                if instances_to_process:
                    app_logger.info(
                        f"Found {len(instances_to_process)} configured {app_type} instances to process"
                    )
                else:
                    # No instances found via get_configured_instances
                    app_logger.warning(
                        f"No configured {app_type} instances found. Skipping cycle."
                    )
                    stop_event.wait(sleep_duration)
                    continue
            except Exception as e:
                app_logger.error(
                    f"Error calling get_configured_instances function: {e}",
                    exc_info=True,
                )
                stop_event.wait(60)
                continue
            ...
            instance_name = app_settings.get(
                "name", f"{app_type.capitalize()} Default"
            )  # Use 'name' or default

            if api_url and api_key:
                app_logger.info(
                    f"Processing {app_type} as single instance: {instance_name}"
                )
        ...
        if not instances_to_process:
            app_logger.warning(
                f"No valid {app_type} instances to process this cycle (unexpected state). Skipping."
            )
            stop_event.wait(sleep_duration)
            continue
        ...
        for instance_details in instances_to_process:
            ...
            if not api_url or not api_key:
                app_logger.warning(
                    f"Missing API URL or Key for instance '{instance_name}'. Skipping."
                )
                continue
            try:
                ...
                if not connected:
                    ...
                    continue
                app_logger.info(
                    f"Successfully connected to {app_type} instance: {instance_name}"
                )
            except Exception as e:
                app_logger.error(
                    f"Error connecting to {app_type} instance '{instance_name}': {e}",
                    exc_info=True,
                )
            ...
            try:
                ...
                app_logger.error(
                    f"Error checking hourly API cap for {app_type}: {e}", exc_info=True
                )
            ...
            app_logger.info(
                f"Using maximum download queue size: {max_queue_size} from general settings"
            )
            ...
            combined_settings.update(
                instance_details
            )  # Add/overwrite with instance specifics (name, url, key)

            # Ensure settings from general.json are consistently used for all apps
            combined_settings["api_timeout"] = settings_manager.get_advanced_setting(
                "api_timeout", 120
            )
            combined_settings["command_wait_delay"] = (
                settings_manager.get_advanced_setting("command_wait_delay", 1)
            )
            combined_settings["command_wait_attempts"] = (
                settings_manager.get_advanced_setting("command_wait_attempts", 600)
            )
            ...
            if hunt_missing_enabled and process_missing:
                try:
                    ...
                    app_logger.error(
                        f"Error during missing processing for {instance_name}: {e}",
                        exc_info=True,
                    )
            ...
            if hunt_upgrade_enabled and process_upgrades:
                try:
                    ...
                    if app_type == "sonarr":
                        ...
                        api_timeout = combined_settings.get("api_timeout", 120)
                        monitored_only = combined_settings.get("monitored_only", True)
                        hunt_upgrade_items = combined_settings.get(
                            "hunt_upgrade_items", 0
                        )
                        upgrade_mode = combined_settings.get("upgrade_mode", "episodes")
                        command_wait_delay = combined_settings.get(
                    ...
                    app_logger.error(
                        f"Error during upgrade processing for {instance_name}: {e}",
                        exc_info=True,
                    )
        ...
        if processed_any_items:
            app_logger.info(
                f"=== {app_type.upper()} cycle finished. Processed items across instances. ==="
            )
        else:
            app_logger.info(
                f"=== {app_type.upper()} cycle finished. No items processed in any instance. ==="
            )
        ...
        next_cycle_time = datetime.datetime.now() + datetime.timedelta(
            seconds=sleep_seconds
        )
        next_cycle_time_str = next_cycle_time.strftime("%Y-%m-%d %H:%M:%S")
        app_logger.info(
            f"Next {app_type.upper()} cycle will begin at {next_cycle_time_str}"
        )
        ...
        while elapsed < sleep_seconds:
            ...
            if stop_event.is_set():
                app_logger.info(
                    "Stop event detected during sleep. Breaking out of sleep cycle."
                )
                break
            ...
            if os.path.exists(reset_file_path):
                try:
                    ...
                    app_logger.info(
                        f"Reset file removed for {app_type}. Starting new cycle now."
                    )
                    break
                except Exception as e:
                    app_logger.error(
                        f"Error processing reset file for {app_type}: {e}",
                        exc_info=True,
                    )
            ...
            if elapsed > 0 and elapsed % 30 == 0:
                app_logger.debug(
                    f"Still sleeping, {sleep_seconds - elapsed} seconds remaining before next cycle..."
                )
...
def reset_app_cycle(app_type: str) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Reset file created for {app_type}. Cycle will reset on next check."
        )
        return True
    except Exception as e:
...
def start_app_threads():
    ...
    configured_apps = {
        app: True for app in configured_apps_list
    }  # Convert list to dict format expected below
    ...
    for app_type, thread in list(app_threads.items()):
        if not thread.is_alive():
            ...
            if configured_apps.get(app_type, False):
                ...
                logger.info(
                    f"Not restarting {app_type} thread as it is no longer configured."
                )
        ...
        while not stop_event.is_set():
            try:
                ...
                if current_time.minute == 0:
                    ...
                    if success:
                        logger.info(
                            f"Successfully reset hourly API caps at {current_time.hour}:00"
                        )
                    else:
                        logger.error(
                            f"Failed to reset hourly API caps at {current_time.hour}:00"
                        )
...
def start_instance_list_generator():
    ...
    logger.debug(
        f"Instance list generator started. Thread is alive: {instance_list_generator_thread.is_alive()}"
    )
...
def start_hourly_cap_scheduler():
    ...
    logger.info(
        f"Hourly API cap scheduler started. Thread is alive: {hourly_cap_scheduler_thread.is_alive()}"
    )


def start_kyhuntarr():
    """Main entry point for KYHUNTARR background tasks."""
    logger.info(f"--- Starting KYHUNTARR Background Tasks v{__version__} --- ")

    # Perform initial settings migration if specified (e.g., via env var or arg)
    if os.environ.get("KYHUNTARR_RUN_MIGRATION", "false").lower() == "true":
        logger.info("Running settings migration from kyhuntarr.json (if found)...")
        settings_manager.migrate_from_kyhuntarr_json()
    ...
    try:
        ...
        shutdown_threads()
        logger.info("--- KYHUNTARR Background Tasks stopped --- ")
