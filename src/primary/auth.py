#!/usr/bin/env python3
"""
Authentication module for KYHUNTARR
...
SESSION_COOKIE_NAME = "kyhuntarr_session"
            ...
            if verify_password(user_data.get("password", ""), password):
                ...
                if two_fa_enabled:
                    ...
                    if otp_code:
                        ...
                        if valid_code:
                            logger.info(
                                f"User '{username}' authenticated successfully with 2FA."
                            )
                            return True, False
                        else:
                            logger.warning(
                                f"Login attempt failed for user '{username}': Invalid 2FA code."
                            )
                            return False, True
                    else:
                        # No OTP code provided but 2FA is enabled
                        logger.warning(
                            f"Login attempt failed for user '{username}': 2FA code required but not provided."
                        )
                        logger.debug(
                            "Returning needs_2fa=True to trigger 2FA input display"
                        )
                        return False, True
                else:
                    # 2FA not enabled, password is correct
                    logger.info(
                        f"User '{username}' authenticated successfully (no 2FA)."
                    )
                    return True, False
            else:
                logger.warning(
                    f"Login attempt failed for user '{username}': Invalid password."
                )
                return False, False
    except Exception as e:
        logger.error(
            f"Error during user verification for '{username}': {e}", exc_info=True
        )

    logger.warning(
        f"Login attempt failed for user '{username}': Username not found or other error."
    )
    return False, False
...
def authenticate_request():
    ...
    if _proxy_bypass_cache["value"] is None or now > _proxy_bypass_cache["expires"]:
        try:
            ...
            _proxy_bypass_cache["value"] = general_settings.get(
    ...
    if not user_exists():
        ...
        if request.path != setup_path and not request.path.startswith(
            (static_path, api_setup_path)
        ):
    ...
    if request.path.startswith(
        (static_path, login_path, api_login_path, setup_path, api_setup_path)
    ) or request.path in (favicon_path, health_check_path):
        return None
    ...
    if local_access_bypass:
        ...
        if forwarded_for:
            ...
            for network in local_networks:
                if possible_client_ip == network or (
                    network.endswith(".") and possible_client_ip.startswith(network)
                ):
                    is_local = True
                    logger.info(
                        f"Forwarded IP {possible_client_ip} is a local network IP (matches {network})"
                    )
                    break
        ...
        if not is_local:
            for network in local_networks:
                if remote_addr == network or (
                    network.endswith(".") and remote_addr.startswith(network)
                ):
                    is_local = True
                    logger.info(
                        f"Direct IP {remote_addr} is a local network IP (matches {network})"
                    )
                    break

        if is_local:
            logger.info(
                f"Local network access from {remote_addr} - Authentication bypassed! (Local Bypass Mode)"
            )
            return None
        else:
            logger.warning(
                f"Access from {remote_addr} is not recognized as local network - Authentication required"
            )
    else:
...
def generate_2fa_secret(username: str) -> Tuple[str, str]:
    ...
    uri = totp.provisioning_uri(name=username, issuer_name="KYHUNTARR")
    ...
    try:
        ...
        logger.error(
            f"Error generating 2FA QR code for user '{username}': {e}", exc_info=True
        )
        raise
...
def verify_2fa_code(username: str, code: str, enable_on_verify: bool = False) -> bool:
    ...
    if not temp_secret:
        logger.warning(
            f"2FA verification attempt for '{username}' failed: No temporary secret found."
        )
        return False
    ...
    if totp.verify(code):
        ...
        if enable_on_verify:
            ...
            if save_user_data(user_data):
                ...
                logger.error(
                    f"Failed to save user data after enabling 2FA for '{username}'."
                )
                return False
        return True
    else:
...
def disable_2fa(password: str) -> bool:
    ...
    if verify_password(user_data.get("password", ""), password):
        ...
        if save_user_data(user_data):
            ...
            logger.error(
                "Failed to save user data after disabling 2FA (password only)."
            )
            return False
    else:
        logger.warning(
            "Failed to disable 2FA (password only): Invalid password provided."
        )
        return False


def disable_2fa_with_password_and_otp(
    username: str, password: str, otp_code: str
) -> bool:
    """Disable 2FA for the specified user, requiring both password and OTP code."""
    user_data = (
        get_user_data()
    )  # Assuming this gets data for the logged-in user implicitly

    # 1. Verify Password
    if not verify_password(user_data.get("password", ""), password):
        logger.warning(
            f"Failed to disable 2FA for '{username}': Invalid password provided."
        )
        return False
    ...
    if not user_data.get("2fa_enabled") or not perm_secret:
        logger.error(
            f"Failed to disable 2FA for '{username}': 2FA is not enabled or secret missing."
        )
    ...
    if not totp.verify(otp_code):
        logger.warning(
            f"Failed to disable 2FA for '{username}': Invalid OTP code provided."
        )
        return False
    ...
    if save_user_data(user_data):
        logger.info(
            f"2FA disabled successfully for '{username}' after verifying password and OTP."
        )
        return True
    else:
...
def change_username(current_username: str, new_username: str, password: str) -> bool:
    ...
    if user_data.get("username") != current_username_hash:
        logger.warning(
            f"Username change failed: Current username '{current_username}' does not match stored hash."
        )
        return False

    if not verify_password(user_data.get("password", ""), password):
        logger.warning(
            f"Username change failed for '{current_username}': Invalid password provided."
        )
        return False
    ...
    if save_user_data(user_data):
        logger.info(
            f"Username changed successfully from '{current_username}' to '{new_username}'."
        )
        return True
    else:
        logger.error(
            f"Failed to save user data after changing username for '{current_username}'."
        )
        return False
...
def get_app_url_and_key(app_type: str) -> Tuple[str, str]:
    ...
    Args:
        app_type: The app type (sonarr, radarr, lidarr)
